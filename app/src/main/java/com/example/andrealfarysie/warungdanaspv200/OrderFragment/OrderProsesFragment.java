...
public class OrderProsesFragment extends Fragment {
    ...
    private void loadMore() {
        ...
        mApiService.listOrder2(sharedPrefManager.getSpBranchId(),2,10,offset).enqueue(new Callback<RespListOrder>() {
            @Override
    ...
    private void firstLoad() {
        itShouldLoadMore = false;
        mApiService.listOrder2(sharedPrefManager.getSpBranchId(),2,10,0).enqueue(new Callback<RespListOrder>() {
            @Override
            public void onResponse(Call<RespListOrder> call, Response<RespListOrder> response) {
                if (response.isSuccessful()){
                    itShouldLoadMore = true;
